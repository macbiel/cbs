...
library(gridExtra)
library(ggplot2)
split(pheno, pheno$batch) -> batches
sapply(batches, function(x) table(x$outcome)) -> outcomes

rownames(outcomes) = paste(sep="", "Outcome #", c(1,2,3,4,5), ": ", rownames(outcomes))
colnames(outcomes) = paste(sep="", "Batch #", colnames(outcomes))

pdf("Bielecki_problem1.pdf")
grid.arrange(tableGrob(outcomes))
dev.off()
```
...
###knitr refused to produce these graphs, so the commands had to be commented out
...
```{r}
#Sort
edata_s=edata[,rownames(pheno[order(pheno$batch),])]
combat_edata_s=combat_edata[,rownames(pheno[order(pheno$batch),])]

###knitr refused to produce these graphs, so the commands had to be commented out
###see PDF files for results
#pdf("Bielecki_problem2_1.pdf")
#heatmap.2(
#          edata_s,
#          main = "Bladder Cancer Data, pre-ComBat, batch-sorted",
#          notecol="black",      # change font color of cell labels to black
#          density.info="none",  # turns off density plot inside color legend
#          trace="none",         # turns off trace lines inside the heat map
#          margins =c(12,9),     # widens margins around plot
...
#          )
#dev.off()
#pdf("Bielecki_problem2_2.pdf")
#heatmap.2(
#          combat_edata_s,
#          main = "Bladder Cancer Data, after cleaning with ComBat, batch-sorted",
#          notecol="black",      # change font color of cell labels to black
#          density.info="none",  # turns off density plot inside color legend
#          trace="none",         # turns off trace lines inside the heat map
#          margins =c(12,9),     # widens margins around plot
...
```{r}
library(pheatmap)
split(pheno, pheno$sample) -> samples
sapply(samples, function(x) paste(sep="", "Batch #", x$batch, ifelse(x$cancer=="Normal", ", Normal", ", Cancer"))) -> samples
paste(rownames(pheno), samples, sep=": ") -> samples
colnames(edata) = samples
colnames(combat_edata) = samples

pdf("Bielecki_problem3_1.pdf")
pheatmap(cor(edata), fontsize = 7)
dev.off()

pdf("Bielecki_problem3_2.pdf")
pheatmap(cor(combat_edata), fontsize = 7)
dev.off()
```
...
```{r}
#Prepare data
B_pheno = pData(bottomly.eset)
B_edata = exprs(bottomly.eset)
sumna <- apply(B_edata, 1, function(x) sum(is.na(x)))
row.variances <- apply(B_edata, 1, function(x) var(x))
row.means <- apply(B_edata, 1, function(x) mean(x))
B_edata <- B_edata[row.variances < 6,]
B_pheno$strain = as.factor(relevel(B_pheno$strain, ref="DBA/2J"))

#ComBat
B_batch = B_pheno$experiment.number

B_combat_edata = ComBat(dat=B_edata, batch=B_pheno$experiment.number, mod=model.matrix(~1, data=B_pheno), par.prior=TRUE, prior.plots=TRUE)


B_mod_un = lm(t(B_edata) ~ as.factor(B_pheno$strain) + as.factor(B_pheno$experiment.number))
tB_mod_un = tidy(B_mod_un)
B_mod_cb = lm(t(B_combat_edata) ~ as.factor(B_pheno$strain))
tB_mod_cb = tidy(B_mod_cb)

B_est_compare <- tibble(
  LinearModel = tB_mod_un %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J") %>% select("estimate") %>% unlist,
  ComBat = tB_mod_cb %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J") %>% select("estimate") %>% unlist)

pdf("Bielecki_problem4_1.pdf")
ggplot(B_est_compare, aes(x=LinearModel, y=ComBat)) +
     geom_point(col="darkgrey", alpha=.5, size=.5) + geom_abline(intercept=0, slope=1, col="darkred") + geom_smooth(method = "lm", se = TRUE)  + theme_bw()
dev.off()

pdf("Bielecki_problem4_2.pdf")
ggplot(tB_mod_cb %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange")
dev.off()
```
...
mod = lm(t(B_edata) ~ as.factor(B_pheno$strain) + as.factor(B_pheno$experiment.number))
mod_tidy <- tidy(mod)

modcombat = lm(t(B_combat_edata) ~ as.factor(B_pheno$strain))
modcombat_tidy <- tidy(modcombat)

est_compare <- tibble(
  LinearModel = mod_tidy %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J") %>% select("estimate") %>% unlist,
  ComBat = modcombat_tidy %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J") %>% select("estimate") %>% unlist)

pdf("Bielecki_problem5_1.pdf")
ggplot(est_compare, aes(x=LinearModel, y=ComBat)) +
     geom_point(col="darkgrey", alpha=.5, size=.5) + geom_abline(intercept=0, slope=1, col="darkred") + geom_smooth(method = "lm", se = TRUE)  + theme_bw()
dev.off()
pdf("Bielecki_problem5_2.pdf")
ggplot(modcombat_tidy %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange")
dev.off()
```

```{r}
#SVA
mod = model.matrix(~as.factor(strain)+as.factor(experiment.number),data=B_pheno)
mod0 = model.matrix(~1, data=B_pheno)
B_edata_nz = B_edata[rowSums(B_edata) != 0,]
B_sva = sva(B_edata_nz, mod, mod0, n.sv=num.sv(B_edata,mod,method="be"))
```
```{r}
#SVA plots

modsva = lm(t(B_edata) ~ as.factor(B_pheno$strain) + B_sva$sv)
modsva_tidy <- tidy(modsva)
...
est_compare <- tibble(
  ...
  SVA = modsva_tidy %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J") %>% select("estimate") %>% unlist)

pdf("Bielecki_problem5_3.pdf")
ggplot(est_compare, aes(x=LinearModel, y=SVA)) +
     geom_point(col="darkgrey", alpha=.5, size=.5) + geom_abline(intercept=0, slope=1, col="darkred") + geom_smooth(method = "lm", se = TRUE)  + theme_bw()
dev.off()
pdf("Bielecki_problem5_4.pdf")
ggplot(modsva_tidy %>% filter(term == "as.factor(B_pheno$strain)C57BL/6J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange")
dev.off()
```